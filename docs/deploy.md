...
| `OBOT_CONDOR_COMMISSION_PER_LEG` | 0.0 | 每腿每次执行佣金$（0=不计）。往返佣金=腿数×2×张数×该值；用于**扣佣净盈亏**（看板卡片/`watch_condor --source engine` 的"扣佣净盈亏/佣金拖累"）。实盘实测老虎美股期权≈**3.22**/腿（HK 现网已设 3.22）。佣金按合约线性，只有**每张权利金加厚**才降拖累%；**加厚靠加翼宽(等比放大风险)或提短腿delta(降胜率)，非高IV**（delta 选腿下信用与 IV 基本无关，见策略总览 §7.8）|
...
## 21. SPXW 升级路径（佣金高效版，资金≥$15k 时）

> **动机**：SPY 散户版铁鹰**被佣金吃到边际**——实盘实测毛权利金 $105、往返佣金 $25.76（拖累 ~25%），回测扣佣后单张净 ~$126/年、佣金占毛利 57%（见 `docs/backtest/2026-07-01-condor-short-delta-16-vs-20.md` 与策略总览 §7.8）。**换到 SPX 指数期权（用 SPXW 周度）**：单张名义 ~$752k（≈10×SPY），同名义只付 1 张合约的佣金 → **佣金拖累从 ~25% 降到 ~5%**，策略由"净≈0"变为可行。

### 21.1 三道闸（2026-07-01 实测，全绿）
| 闸 | 状态 | 证据 |
|---|---|---|
| 行情数据 | ✅ | `SPXW` 实时 bid/ask 可得（3/3）；`SPX` 月度无实时报价，用 **SPXW 周度** |
| 交易权限 | ✅ | 实盘发不可成交 SPXW 垂直价差 → 券商接受 `HELD`/filled0（非 REJECTED），立即撤单零成本 |
| 资金 | ✅(≥$15k) | 窄翼 SPXW 铁鹰可行，见下方尺寸表 |

### 21.2 尺寸表（16Δ，SPX≈7520，占用=最大亏损）
| 翼宽(点) | 毛权利金 | 最大亏损 | 佣金拖累 | 占$15k | 占$20k |
|---|---|---|---|---|---|
| $10 | ~$314 | ~$686 | 10.2% | 5% | 3% |
| **$20（推荐）** | ~$616 | ~$1,384 | **5.2%** | 9% | 7% |
| $25 | ~$762 | ~$1,738 | 4.2% | 12% | 9% |
| $50（满宽）| ~$1,448 | ~$3,552 | 2.2% | 24% | 18% |
> **10×名义是 SPX 每张自带的、与翼宽无关**——所以窄翼($20)也能把拖累压到 5.2%，不必满$50翼/$50k。$50翼在 $15k 上占用 24%、太集中，不推荐。

### 21.3 切换与验证步骤（资金到位后）
```bash
cd ~/tiger-quantitative-trade
# 1) 改 .env
#    OBOT_CONDOR_UNDERLYING=SPXW
#    OBOT_CONDOR_WING_WIDTH=20        # 或 25；占用≈账户9%~12%
#    OBOT_MAX_QTY=1                    # 先 1 张
#    OBOT_OPEN_ON_START=false / OBOT_ALLOW_LIVE_AUTO_OPEN=false（不变）
sudo docker compose up -d --force-recreate   # 仅重载 .env

# 2) 先跑通一张 combo 全链路（人工 approve，眼盯）：确认
#    - SPXW 链能选到 16Δ 短腿 + 翼（PM 结算、链格式）
#    - CUSTOM 4 腿原子成交、无半成交
#    - 真实佣金（$4/腿为估值，首张揭晓）
#    - 平仓翻转腿归 0、无残腿（SPY 也待验的那项）
# 3) 通过后再常态化
```

### 21.4 仍要清醒的点
- **佣金 $4/腿是估的**：SPX 交易所/监管费可能更高；首张实测确认。即便往返 $50，$20翼拖累也才 ~8%，仍碾压 SPY。
- **集中度**：$20翼单笔风险 ~$1,384 ≈ 账户 9%，单仓+负偏，一次 max loss = −9%；别过度交易、别同时多仓。
- **SPXW 是 PM 结算周度**：引擎 DTE/到期处理需实测跑通（首张验证的目的之一）。
- **回测仍乐观**（无 skew/滑点），真实净更低；SPX 尾部同样被低估。
- SPX 额外真实利好未计入：**1256 合约 60/40 税优 + 现金结算（无指派/pin）**。
